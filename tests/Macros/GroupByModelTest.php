...
it('can group a collection by a model with a callable', function () {
    ...
    $grouped = $collection->groupByModel(fn ($item) => $item['model']);

    expect($grouped)->toHaveCount(2);

    foreach ($expected as $i => $group) {
        expect($grouped[$i][0])->toEqual($group[0])
            ->and($grouped[$i][1]->toArray())->toEqual($group[1]);
    }
});

it(
    'can group a collection by a model with callable and custom key names',
    function () {
        [$model1, $model2, $collection] = getDummies();
...
it(
    'can group a collection by a model with a key and a custom items key',
    function () {
        [$model1, $model2, $collection] = getDummies();
